...
- AI page: https://laravel-form-builder-assigment.onrender.com/ai
- Access: open public access, no credentials required
- Demo credentials: not required; the hosted site is publicly accessible
- Deployed via Render with a web service and a queue worker

## Architecture overview

- Laravel 13 backend with Blade and Livewire for dynamic UI
- JSON schema stored in `forms.schema` as the single source of truth
- MySQL-backed database with migrations, seeders, and indexed lookup
- Queued OpenAI AI generation tasks using `ai_tasks` and `queue:work`
- Docker + Docker Compose for local development and deploy readiness
- `public/build` assets generated by Vite and served through Laravel

## Remaining limitations

- No production authentication or role-based access control is implemented.
- Import parsing is deterministic only and does not currently use AI-assisted inference for ambiguous documents.
- There is no spam/rate-limiting protection for public form submissions.
- Automated tests and CI configuration are not included.
...
The app is deploy-ready and a live demo is available at the URL above.
...
## Database model summary and schema

The app stores form configuration as a JSON schema inside `forms.schema`. This schema drives the form builder UI, field settings, validation rules, and public form rendering.
...
- `forms` is the core form registry table. `public_uuid` is used for public fill links and is unique.
- `form_submissions` stores submissions attached to a form and includes a `search_text` field for fast search.
- `ai_tasks` records AI generation requests and metadata for monitoring and retries.

## API endpoints

- `GET /` — view the list of forms
- `GET /forms/create` — open the manual form builder
- `GET /forms/{form}/edit` — edit an existing saved form
- `GET /forms/{form}/submissions` — list submissions for a saved form
- `GET /forms/{form}/submissions/export` — export submissions as CSV
- `GET /forms/{publicUuid}` — public form fill page for published forms
- `POST /forms/{publicUuid}` — submit a public form response
- `GET /import` — upload Word or Excel files for import preview
- `POST /import/preview` — parse an import file and show editable preview
- `GET /ai` — AI form generation and task status page
- `POST /ai/generate` — queue an AI form generation/edit request
